...
class EnderecoServicoTest {
    ...
    fun setup() {
        ...
        enderecoServico = EnderecoServico(enderecoRepositorio, usuarioServico = usuarioServico)
    }
    ...
    fun deletarEnderecoSucesso() {
        ...
        enderecoServico.deletarPorId(enderecoId, usuarioId)

        verify(enderecoRepositorio, times(1)).deleteById(enderecoId)
    }

    @Test
    @DisplayName("Deve lançar exceção ao tentar deletar endereço por ID inexistente")
    fun deletarEnderecoIdInexistente() {
        val usuarioId = 1
        val enderecoId = 1
        val usuario = Usuario(id = usuarioId, nome = "Enzo")
